            ...
            for iteration in tqdm(range(int(X.shape[0] / self.batch_size) + 1)):
                ...
                if (iteration) % log_iters == 0:
                    ...
                    if dev_score > best_score:
                        ...
                        self.save_model(save_path)
                        print(f'Best accuracy has been updated: {best_score:.4f} --> {dev_score:.4f}')
                        best_score = dev_score
                    else:
                        stop_count += 1
                        # at least train a whole epoch
                        if epoch == 0:
                            stop_count = 0
                        if stop_count >= stop_threshold:
                                # tackle the parameters
                                self.best_score = best_score
                                self.dev_scores.append(dev_score)
                                self.dev_loss.append(dev_loss)
                                # resave the model
                                self.model.load_model(os.path.join(save_dir, 'saved_model.pkl'))
                                self.model.save_model(os.path.join(save_dir, 'best_model.pkl'))
                                # information
                                print(f'Early stopping triggered: Model has not been improved during {stop_count * log_iters: d} iterations')
